...
function Dijkstra(map,height::Int64,width::Int64,visited::Matrix{Bool},dist::Matrix{Int64},prio::PriorityQueue{Tuple{Int64,Int64},Int64},pred::Matrix{Tuple{Int64,Int64}},A::Tuple{Int64,Int64}) # Function that implements Dijkstra's
    ...
    while visited[A[1],A[2]]!=true && length(prio)!=0 
        ...
        visited[x,y]=true
        nb_visit+=1
        # Update dist,pred and priority value of the adjacent point
        # of the picked point only if the current
        ...
        if inMatrix((x,y+1),height,width) && visited[x,y+1]==false && cost[matrx[x,y+1]]!=99999
            ...
            if dn<dist[x,y+1]
                prio[(x,y+1)]=dn #Change an existing item’s priority to a lower number
                dist[x,y+1]=dn
                pred[x,y+1]=p
            end
        end
    ...
    return nb_visit #return the number of states evaluated
end

function reconstruct_path(prev::Matrix{Tuple{Int64,Int64}},D::Tuple{Int64,Int64},A::Tuple{Int64,Int64}) #build the path
    ...
    return path #returns a vector containing all the points of the shortest path
end
...
function toRGB(matrx::Matrix{Char},matrpd::Matrix{Bool},pred::Vector{Tuple{Int64,Int64}})
...
function algoDijkstra(map,D::Tuple{Int64,Int64},A::Tuple{Int64,Int64}) #main function
